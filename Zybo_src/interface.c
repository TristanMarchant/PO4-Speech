...
void interface_init()
{
	...
	accelerator_data = (unsigned int *)0x43C10000;
}
...
{
    while(accelerator_port[1]==0)  //Read a response from P2
    {}
}
...
void print_bram_contents()
{
    ...
    for (i=DMA_TRANSFER_NUMBER_OF_WORDS-4; i>=0; i-=4)
        xil_printf("%08x %08x %08x %08x\n\r",accelerator_data[i+3], accelerator_data[i+2], accelerator_data[i+1], accelerator_data[i]);
}
